...
export default class Panchang extends Component {
  _isMounted = false;
  constructor(props) {
    super(props);
    this.state = {
  ...
  fetchDate = () => {
    ...
    // this.setState({
    //   dateF: this.state.Date + '-' + this.state.month + '-' + this.state.year,
    // });
  ...
  async postData() {
    try {
      ...
      var val = {
        ...
        hour: Number(this.state.birthHour),
        min: this.state.birthMinutes,
        lat: this.state.lat,
        lon: this.state.lon,
        tzone: this.state.tzone,
      };
      this.props.navigation.navigate('panchangData', {
        data: val,
      });
    } catch (error) {
      console.log(error);
